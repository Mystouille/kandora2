...
export async function getOrGenerateUserAgent(): Promise<string> {
  const [apiConfig] = await MajsoulConfig.find();
...
export async function main() {
  ...
  const majsoulId = await api.getAccountIdFromFriendId("81233766");
  if (majsoulId != undefined) {
    const accountInfo = await api.fetchAccountInfo(majsoulId);
